...
//	}
	...
	public static Utf8String decodeUtf8String(String data) {
		byte[] bytes = Numeric.hexStringToByteArray(data);
			return new Utf8String(new String(bytes, StandardCharsets.UTF_8));
	}
...
//	private static <T extends Type> T decodeArrayElements(String input, int offset, TypeReference<T> typeReference,
//			int length, BiFunction<List<T>, String, T> consumer) {
//
//		try {
...
//
//				for (int i = 0, currOffset = offset; i < length; i++, currOffset += getSingleElementLength(input,
//						currOffset, cls) * MAX_BYTE_LENGTH_FOR_HEX_STRING) {
